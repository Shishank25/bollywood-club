...
// Configured specifically for the Corporate Events page slots
const CORPORATE_SLOTS = [
  { id: 'hero-media', label: '🎬 Hero Media', description: 'Main background video or image at the top of the corporate page', folder: 'corporate' },
  { id: 'cap-1', label: '✨ Venue Setup Card', description: 'Media for the Venue Setup capability card', folder: 'corporate' },
  { id: 'cap-2', label: '✨ AV & Tech Card', description: 'Media for the AV & Tech Support capability card', folder: 'corporate' },
  { id: 'cap-3', label: '✨ Premium Catering Card', description: 'Media for the Premium Catering capability card', folder: 'corporate' },
  { id: 'cap-4', label: '✨ Event Planning Card', description: 'Media for the End-to-End Planning capability card', folder: 'corporate' },
  { id: 'form-media', label: '📝 Form Media', description: 'Image or video displayed next to the corporate inquiry form', folder: 'corporate' },
];
...
export default function CorporatePageEditorPage() {
  ...
  useEffect(() => {
    fetchCorporatePageMedia();
  }, []);

  const fetchCorporatePageMedia = async () => {
    try {
      setLoading(true);
      // Fetching specifically for the /corporate route
      const res = await fetch('/api/media?page=/corporate');
      if (!res.ok) throw new Error('Failed to fetch corporate page media');
      
      const data = await res.json();
      setMediaAssets(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error loading corporate page data');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-slate-400">Loading corporate page editor...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Page Title */}
      <div>
        <h2 className="text-3xl font-bold text-white mb-2">Edit Corporate Events Media</h2>
        <p className="text-slate-400">Configure the hero and form media assets for your corporate offerings</p>
      </div>
      ...
      <div className="grid gap-6">
        {CORPORATE_SLOTS.map((slot) => (
          <MediaEditorCard 
            key={slot.id}
            slotConfig={slot}
            initialData={mediaAssets[slot.id]}
            onRefresh={fetchCorporatePageMedia}
          />
        ))}
      </div>
    </div>
...
function MediaEditorCard({ 
  ...
  const handleSave = async () => {
    try {
      ...
      // Ensure we hit the Corporate route
      fd.append('pageRoute', '/corporate');
